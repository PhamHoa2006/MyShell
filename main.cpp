...
void print_startup_info() {
    ...
    cout << "Process commands    : start_foreground, start_background, terminate, suspend, resume, list_background, list_all_processes" << endl;
    cout << "Utility & Env cmds  : time, date, path, addpath, set_env, unset_env, print_env, list_env, run" << endl;
    cout << "==============================================================================" << endl;
}
...
void execute_command(const string& command, const vector<string>& args){
    ...
    // --- FILE COMMANDS (Role 2) ---
        ...
        if (args.size() >= 2) envMgr.setEnv(args[0], args[1]);
        else cerr << "Usage: set_env <variable_name> <value>\n";
    } else if (command == "unset_env") {
        if (!args.empty()) envMgr.unsetEnv(args[0]);
        else cerr << "Usage: unset_env <variable_name>\n";
    } else if (command == "print_env") {
        if (!args.empty()) envMgr.printEnv(args[0]);
        else cerr << "Usage: print_env <variable_name>\n";
    } else if (command == "list_env") {
        envMgr.listAllEnv();
    } else if (command == "path") {
        envMgr.printPath();
    } else if (command == "addpath") {
        if (!args.empty()) envMgr.addPath(args[0]);
        else cerr << "Usage: addpath <directory_path>\n";
    } else if (command == "run") {
    ...
    // --- EXIT ---
        ...
        cout << "Unknown command: " << command << ". Type 'help' for the manual." << endl;
    }
}
