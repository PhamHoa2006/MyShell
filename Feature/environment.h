...
//
// Key C functions:
//   getenv("VAR")              — Get an environment variable value (returns
//   char*) _putenv_s("VAR", "value")  — Set an environment variable
//   (Windows-specific)
//
...
public:
  ...
  void addPath(const std::string &newPath) {
    ...
    if (std::find(searchPaths.begin(), searchPaths.end(), newPath) ==
        searchPaths.end()) {
